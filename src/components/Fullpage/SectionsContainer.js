...
const SectionsContainer = React.createClass({
    ...
    _mouseWheelHandler() {
        ...
        //当最后一屏不足一屏时  假设高为480 
        let maxPosition = 0 - ((this.props.anchors.length - 1) * this.state.windowHeight + 480);
        ...
        //当最后一屏不足一屏时  假设高为480 anchors为'sectionFive'
        if (index === 'sectionFive') {
            position = this.state.sectionScrolledPosition + (delta * 480)
        }
    ...
    _handleSectionTransition(index) {
        ...
        //当最后一屏不足一屏时  假设高为480 anchors为'sectionFive'
        if (index === (this.props.anchors.length - 1)) {
            position = 0 - ((this.props.anchors.length - 2) * this.state.windowHeight + 480);
        }
